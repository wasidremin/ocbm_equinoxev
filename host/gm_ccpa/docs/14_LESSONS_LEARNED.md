...
- `versionCode` is **not** `git rev-list --count HEAD` on this shallow mirror (that is 8). Play has already consumed 8–14 and 20–21. **High-water mark: versionCode 28** (`4.0+fw-lock`, 2026-09-22). Next Play upload must be **> 28**; a reuse is refused with `403 Version code N has already been used.`
