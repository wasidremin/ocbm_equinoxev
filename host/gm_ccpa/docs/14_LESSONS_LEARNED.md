...
- `versionCode` is **not** `git rev-list --count HEAD` on this shallow mirror (that is 8). Play has already consumed 8–14 and 20–31. **High-water mark: versionCode 31** (`4.0+aaaa-only`, 2026-09-22, `automotive:internal` COMPLETED). Next Play upload must be **> 31**; a reuse is refused with `403 Version code N has already been used.`
