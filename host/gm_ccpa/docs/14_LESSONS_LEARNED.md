...
- `versionCode` is **not** `git rev-list --count HEAD` on this shallow mirror (that is 8). Play has already consumed 8–14 and 20–32. **High-water mark: versionCode 32** (`4.0+no-nsd-advert`, 2026-09-22, `automotive:internal` COMPLETED). Next Play upload must be **> 32**; a reuse is refused with `403 Version code N has already been used.`
