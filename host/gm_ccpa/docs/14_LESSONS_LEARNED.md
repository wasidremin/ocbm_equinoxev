...
- `versionCode` is **not** `git rev-list --count HEAD` on this shallow mirror (that is 8). Play has already consumed 8–14 and 20–21. **High-water mark: versionCode 27** (`4.0+fw-live`, 2026-09-21). Next Play upload must be **> 27**; a reuse is refused with `403 Version code N has already been used.`
