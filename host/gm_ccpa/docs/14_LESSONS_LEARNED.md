...
| Phone dials back to `:7011` (pair-verify onward) | Silverado: proven | **NOT reached.** 35 s + 26 s windows, connect-out accepted each time, phone queries our PTR/SRV/TXT/A and we answer, no inbound. Matches the 2026-08-27 "sees the receiver, will not connect" signature (`12_OBSERVED_FLOW.md` Phase 4/5) — cleared there by reconnecting BT from the iPhone. **New VCU-only suspect:** GM firewalling inbound TCP from hotspot clients to the HU on `ap_br_swlan0` (HU→phone works; phone→HU `:7011` unproven) |

**Still open on Equinox, in order:**

1. **Get the dial-back.** First the documented remedy: with the app in `awaiting dial-back`, on the iPhone tap `CarLink-6754` under Settings ▸ Bluetooth (or pick the car under Settings ▸ General ▸ CarPlay), give it 60 s+, upload. If it never dials in across a few tries, test inbound reachability of `:7011` on `ap_br_swlan0` — the VCU may only admit its own CarPlay ports from hotspot clients.
2. **Make iOS auto-join.** Copy SSID and passphrase from the HU's Hotspot page character-for-character into the app (`Wi-Fi: … tap to edit`) and check the hotspot's security mode; auto-join failing with a correct SSID means passphrase or WPA3. Ship the app with these fields **blank** so the probe's empty-SSID refusal forces entry — the Silverado defaults cost the first three drives.
