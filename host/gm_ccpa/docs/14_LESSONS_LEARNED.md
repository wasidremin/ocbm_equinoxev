...
- `versionCode` is **not** `git rev-list --count HEAD` on this shallow mirror (that is 8). Play has already consumed 8–14 and 20–21. **High-water mark: versionCode 22** (`4.0+persist-creds`, 2026-09-21). Next Play upload must be **> 22**; a reuse is refused with `403 Version code N has already been used.`
...
1. **Drive versionCode 22 with the SSID entered once.** This is the clean AAAA test 21 was meant to be. Expect the phone to resolve AAAA and `>>> INBOUND CONTROL CONNECTION from fe80::…%ap_br_swlan0`. If it still dials IPv4 only, or dials IPv6 and is still dropped, the VCU's `ip6tables` differs from gminfo37's (`-i br0 -s fe80::/64 -j ACCEPT`) and the next move is finding which ports/paths GM's own `:7000` receiver is admitted on (Safari to `http://gmccpa-rx.local:7000/info` from the phone answers that in one go: a response = IPv4 `:7000` is allowlisted).
