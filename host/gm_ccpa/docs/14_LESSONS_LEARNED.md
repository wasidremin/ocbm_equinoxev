...
| Phone joins the vehicle hotspot, advertises `_carplay-ctrl._tcp` | Silverado: proven (`br0`) | **NOT reached.** Receiver on `ap_br_swlan0` (hotspot up; last night it was `wlan0`, i.e. hotspot off). 45 s after each of two handoffs: no `_carplay-ctrl` resolve, no inbound. Phone-on-Wi-Fi is the open question |

**Still open on Equinox, in order:**

1. **Prove the phone actually joins `myChevrolet 32D4`.** The box side is byte-identical to the Silverado flow, so the suspects are all vehicle-side: passphrase typed into the app vs the HU's Hotspot page (case-sensitive), hotspot security (VCU may be WPA3-only), hotspot actually on before the handoff, and whether GM's `ap_br_swlan0` passes mDNS between the HU and hotspot clients. Check on the iPhone (Settings ▸ Wi-Fi during the handoff) and the HU's hotspot client list; `ip neigh show dev ap_br_swlan0` against the phone's Wi-Fi MAC from `0x4E0E DeviceTransportIdentifier param0` is the definitive app-side test — worth logging at the 45 s deadline.
